...
class BlockingReaderWriterCircularBuffer {
   ...
   public:
    ...
    // thread. No exception guarantee (state will be corrupted) if constructor
    // of T throws.
    ...
    // Blocks the current thread until there's enough space to enqueue the given
    // item, then enqueues it (via move, if possible). Thread-safe when called
    // by producer thread. No exception guarantee (state will be corrupted) if
    ...
    // Blocks the current thread until there's enough space to enqueue the given
    // item, or the timeout expires. Returns false without enqueueing the item
    // if the timeout expires, otherwise enqueues the item (via copy) and
    // returns true. Thread-safe when called by producer thread. No exception
    ...
    // Blocks the current thread until there's enough space to enqueue the given
    // item, or the timeout expires. Returns false without enqueueing the item
    // if the timeout expires, otherwise enqueues the item (via move, if
    // possible) and returns true. Thread-safe when called by producer thread.
    // No exception guarantee (state will be corrupted) if constructor of T
    // throws.
    ...
    // No exception guarantee (state will be corrupted) if assignment operator
    // of U throws.
    template <typename U>
    ...
    // guarantee (state will be corrupted) if assignment operator of U throws.
    template <typename U>
