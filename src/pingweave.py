    ...
    try:
        while True:
            ...
            if config["control_host"] in local_ips:
                ...
                if key not in running_processes or not (
                    running_processes[key] and running_processes[key].poll() is None
                ):
                        ...
                        if protocol in protocol_to_process_names:
                            for pname in protocol_to_process_names[protocol]:
                                ...
                                if key not in running_processes or not (
                                    ...
                                    and running_processes[key].poll() is None
                                ):
                                    f_to_run = processes_to_run[key[1]] + [ip]
                                    if key[2] in ["roce", "ib"]:
                                        f_to_run = f_to_run + [key[2]]
                                    running_processes[key] = start_process(
                                        f_to_run, key[1], logger
                                    )
                                    send_message_via_http(
                                        f"{key[1]} starts",
                                        "/alarm",
        ...
        send_message_via_http(
            msg, "/alarm", config["control_host"], config["collect_port_http"], logger
        )
